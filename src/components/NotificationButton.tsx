...
const NotificationButton = () => {
  ...
  <summary className="m-1 btn bg-slate-500 border focus:bg-slate-900">
  ...
  <div className="py-2 w-9/12 shadow-xl">
      {
        notifications.map((item) => (
             <a href="#" key={item.ID} className="flex items-center px-4 py-3 -mx-2 transition-colors duration-300 transform border-b  bg-slate-700  rounded-md">
                ...
                <p className="mx-2 text-sm text-white"><span className="font-bold capitalize" >{item.Creator.Username}</span> comento tu publicación <span className="text-blue-500 hover:underline " >"{item.Post.Description}"</span> {item.CreatedAt.slice(0,10)} </p>
            </a>
